...
def extract_wps(raw: bytes) -> str:
    """将 WPS 文件通过 LibreOffice 转换为 docx，再提取文本

    WPS 文件本质上是类似 Office 的格式，LibreOffice 可以打开并转换
    """
    if not os.path.exists(LIBREOFFICE_BIN):
        raise ValueError("libreoffice_not_found")

    with tempfile.TemporaryDirectory() as tmpdir:
        input_path = os.path.join(tmpdir, "input.wps")
        ...
        if not os.path.exists(converted):
            raise ValueError("wps_convert_no_output")
