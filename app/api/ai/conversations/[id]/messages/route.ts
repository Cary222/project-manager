...
import {
  onNodeStart,
  onNodeEnd,
} from "@/features/ai/lib/timeline/timeline-adapter";
...
import {
  speculationCache,
  shouldSpeculate,
} from "@/features/ai/search/speculation-cache";
...
import {
  searchKnowledge,
  setSearchKnowledgeViewer,
  setSearchKnowledgeConversationId,
} from "@/features/ai/tools/search-knowledge";
import {
  searchStructured,
  setSearchStructuredViewer,
} from "@/features/ai/tools/search-structured";
import {
  shouldUseWebSearch,
  shouldUseRag,
} from "@/features/ai/search/detector";
import { HumanMessage, AIMessage } from "@langchain/core/messages";
import {
  loadRuntimeState,
  saveRuntimeState,
} from "@/features/ai/core/runtime/conversation-state-store";
...
import {
  agentGraph,
  type AgentState,
} from "@/features/ai/agents/conversation/agent";
...
function extractSourcesFromToolResults(
  ...
  for (const { toolName, output } of toolResults) {
    ...
    if (
      toolName === "searchStructured" &&
      o._debug === "structured_with_sources" &&
...
const messageSchema = z.object({
  ...
  conversationHistory: z
    ...
    .optional(),
  mode: z
    .enum(["auto", "search", "chat", "web", "image"])
    .optional()
    .default("auto"),
...
async function validateInputImageOwnership(
  ...
  Array<{ id: string; storageType: string; mimeType: string | null }>
> {
...
export async function POST(
  ...
  try {
    ...
    console.log(
      `[AI-MSG] parsed message="${message.slice(0, 80)}" mode=${mode}`,
    );
    ...
    if (process.env.USE_LANGGRAPH === "true") {
      ...
      let validatedInputImages: Awaited<
        ReturnType<typeof validateInputImageOwnership>
      > = [];
      const inputImageIds = body.inputImageIds;
      // #H1 instrument: 确认 chat 模式下 inputImageIds 是否为空
      console.log(
        `[DEBUG:H1] inputImageIds received: count=${inputImageIds?.length ?? 0} values=${JSON.stringify(inputImageIds)}`,
      );
      if (inputImageIds && inputImageIds.length > 0) {
        try {
          ...
          if (msg.startsWith("UNAUTHORIZED_INPUT_IMAGE")) {
            const missingIds = msg
              .replace(/^UNAUTHORIZED_INPUT_IMAGE:\s*/, "")
              .split(",")
              .filter(Boolean);
            console.warn(
          ...
          if (msg.startsWith("NON_IMAGE_INPUT")) {
            const nonImageIds = msg
              .replace(/^NON_IMAGE_INPUT:\s*/, "")
              .split(",")
              .filter(Boolean);
            console.warn(
    ...
    let legacyValidatedInputImages: Array<{
      id: string;
      storageType: string;
    ...
    const autoNeedsWeb =
      mode === "auto" && (useWebSearch || shouldUseWebSearch(message));
    ...
    if (mode === "auto" && shouldSpeculate(message)) {
      console.log(
        `[AI-MSG] speculation triggered for "${message.slice(0, 50)}"`,
      );
      ragPromise
        .then((context) => {
    ...
    const maxSteps = maxStepsForMode(
      mode as "auto" | "web" | "search" | "chat",
    );
    ...
    if (autoNeedsWeb) {
      ...
      resolvedMaxSteps = 15;
      console.log(
        `[AI-MSG] intent=WEB_SEARCH detected, useWebSearch=${useWebSearch} city=${geoCity ?? "none"}`,
      );
    } else if (mode === "auto") {
    ...
    const result: any = await withStreamAgnesDynamicModel((model) =>
      streamText({
        ...
        messages: [
          ...messages,
          { id: "current", role: "user", content: message },
        ],
        tools: resolvedTools,
        stopWhen: stepCountIs(resolvedMaxSteps),
        onFinish: async ({ text }) => {
          console.log(
            `[AI-MSG] onFinish textLen=${text?.length ?? 0} preview="${(text ?? "").slice(0, 80)}"`,
          );
          const context = await ragPromise;
          const sources = isRagToolEnabled
            ? extractSourceReferences(context.results)
            : [];
          await appendMessage(
            conversationId,
            "assistant",
    ...
    const responseStream = new ReadableStream({
      async start(controller) {
        ...
        try {
          console.log(
            `[AI-SSE] start conv=${conversationId} toolNames=[${resolvedTools ? Object.keys(resolvedTools).join(",") : "none"}]`,
          );
          enqueueData({
            type: "conversation",
            id: conversationId,
          ...
          try {
            ...
            console.log(
              `[AI-SSE] result.text len=${fullText?.length ?? 0}, preview="${(fullText ?? "").slice(0, 80)}"`,
            );
            if (totalTextChars === 0 && fullText && fullText.length > 0) {
              console.log(
                `[AI-SSE] no deltas emitted, falling back to result.text`,
              );
            ...
            console.log(
              `[AI-SSE] result.text read failed: ${textErr instanceof Error ? textErr.message : String(textErr)}`,
            );
          }
        } catch (err) {
        ...
        console.log(
          `[AI-SSE] end conv=${conversationId} textCharsSent=${totalTextChars}`,
        );

        // Extract and send sources from tool results and RAG
        try {
          const ragContext = await ragPromise;
          ragSources = isRagToolEnabled
            ? extractSourceReferences(ragContext.results)
            : [];
        } catch (e) {
          console.log(`[AI-SSE] failed to get RAG sources: ${e}`);
        }

        const allSources = extractSourcesFromToolResults(
          toolResults,
          ragSources,
        );
...
async function handleLangGraphRequest(
  ...
  console.log(
    `[AI-LangGraph] start conv=${conversationId} message="${message.slice(0, 80)}" mode=${mode}`,
  );
  ...
  if (validatedInputImages.length > 0) {
    try {
      const { resolveCurrentInputImages } = await import(
        "@/features/ai/core/images/resolve-current-input-images"
      );
      currentInputImageUrls =
        await resolveCurrentInputImages(validatedInputImages);
      console.log(
        `[AI-LangGraph] current input images resolved: ${currentInputImageUrls.length}`,
      );
    } catch (err) {
      currentImageResolveError =
        err instanceof Error ? err.message : String(err);
      console.error(
        `[AI-LangGraph] failed to resolve current input images:`,
        err,
      );
    }
  }
  ...
  if (conversationHistory && conversationHistory.length > 0) {
    ...
    if (userHistoryIds.length > 0) {
      try {
        const { resolveHistoryInputImages } = await import(
          "@/features/ai/core/images/resolve-history-input-images"
        );
        ...
        console.error(
          `[AI-LangGraph] failed to resolve history input images:`,
          err,
        );
      }
    }
  ...
  console.log(
    `[DEBUG:H2] currentInput: text="${message.slice(0, 40)}" imageUrls=${JSON.stringify(currentInputImageUrls)} historyImageUrlsSize=${historyImageUrls.size}`,
  );
  const langgraphMessages = buildMessages({
    history: conversationHistory ?? [],
    currentInput: {
      text: message,
      ...(currentInputImageUrls.length > 0
        ? { imageUrls: currentInputImageUrls }
        : {}),
    },
    ...
    async start(controller) {
      ...
      try {
        ...
        const initialState = {
          ...
          resolvedEntities: (runtimeState?.human?.resolvedEntities ??
            null) as AgentState["resolvedEntities"],
          toolResults: {},
          ...
          lastMentionedUser: runtimeState?.semantic?.lastMentionedUser
            ? {
                id: runtimeState.semantic.lastMentionedUser.id,
                name: runtimeState.semantic.lastMentionedUser.name,
              }
        ...
        if (!graphStream) {
          ...
          for await (const chunk of graphStream) {
            ...
            for (const [nodeName, nodeOutput] of Object.entries(typedChunk)) {
              ...
              if (
                nodeOutput.toolResults &&
                typeof nodeOutput.toolResults === "object"
              ) {
                const results = nodeOutput.toolResults as Record<
                  string,
                  unknown
                >;
              ...
              if (
                ...
                if (alreadySentPendingConfirmation) {
                  ...
                  console.log(
                    `[AI-LangGraph] skipping duplicate pending_confirmation`,
                  );
                } else {
                  alreadySentPendingConfirmation = true;
                  console.log(
                    `[AI-LangGraph] pending_human_action: ${pha.candidates.length} candidates entityType=${pha.entityType}`,
                  );
                  enqueueData({
                ...
                if (!alreadySentPendingConfirmation) {
                  alreadySentPendingConfirmation = true;
                  console.log(
                    `[AI-LangGraph] pending_human_action: workflow approval reason="${wfa.reason ?? ""}"`,
                  );
                  enqueueData({
                    type: "pending_confirmation",
                  ...
                  const workflowName = nameMatch
                    ...
                    : wfa.entityType;
                  const workflowType =
                    wfa.entityType === "workflow"
                      ? ((
                          nodeOutput.workflowMatch as
                            | { type?: string }
                            | undefined
                        )?.type ?? "unknown")
                      : wfa.entityType;
              ...
              if (nodeName === "humanConfirmation") {
                ...
                const resEnt = nodeOutput.resolvedEntities as
                  | Record<string, unknown>
                  | undefined;
                if (
                  resEnt?.user ||
                  resEnt?.weekly_report ||
                  resEnt?.ticket ||
                  resEnt?.project
                ) {
                ...
                const lmu = nodeOutput.lastMentionedUser as
                ...
                if (lmu) {
                  ...
                  console.log(
                    `[AI-LangGraph] captured lastMentionedUser from humanConfirmation: ${lmu.name}`,
                  );
                }
              }
          ...
          if (interruptReason) {
            if (capturedPendingHumanAction) {
              ...
              console.log(
                `[AI-LangGraph] stored pending human action for conv=${conversationId}`,
              );
            }
        ...
        if (
          searchKnowledgeResult?.output &&
          typeof searchKnowledgeResult.output === "object"
        ) {
          const skOutput = searchKnowledgeResult.output as Record<
            string,
            unknown
          >;
        ...
        if (lastResponse.startsWith("[WORKFLOW_MATCH:")) {
          ...
          if (match) {
            ...
            console.log(
              `[AI-LangGraph] workflow match detected: ${workflowType}`,
            );
            enqueueData({
              type: "workflow_match",
              workflowType,
        ...
        console.error(
          `[AI-LangGraph] stream error: ${errMsg}${isRecursion ? " (saving pending state)" : ""}`,
        );
