...
import type {
  ...
  PluginScope,
  PluginsResponse,
} from "@/features/ai/ui/ai-workspace/lib/api-types";
...
async function readPlugins(cwd: string): Promise<PluginsResponse> {
  ...
  const projectTrust = await getProjectTrustStatus(cwd, agentDir);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const agentDir = getAgentDir();
    const projectTrust = await getProjectTrustStatus(body.cwd, agentDir);
