---
source: https://notes.kamacoder.com/llm/transformer/fnn_code.html
category: transformer
scraped_at: 2026-08-14T07:54:40.685Z
---

# [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E6%89%8B%E6%92%95ffn-transformer%E5%89%8D%E9%A6%88%E7%BD%91%E7%BB%9C%E4%BB%A3%E7%A0%81%E5%AE%9E%E7%8E%B0) 手撕FFN：Transformer前馈网络代码实现

[![卡码笔记](https://file1.kamacoder.com/i/web/2025-08-14kamabij.jpg)\\
\\
公众号@卡码笔记原创\\
\\
2026-05-25·全文 2095 字\\
\\
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#ffn-%E7%9A%84%E7%BB%93%E6%9E%84%E6%9C%89%E5%A4%9A%E7%AE%80%E5%8D%95) FFN 的结构有多简单？

很简单： **两层线性变换，中间夹一个激活函数。**

公式写出来长这样：

FFN(x)=Activation(x⋅W1+b1)⋅W2+b2\\text{FFN}(x) = \\text{Activation}(x \\cdot W\_1 + b\_1) \\cdot W\_2 + b\_2
...
以原始 Transformer 论文为例，d\_model = 512，中间层 d\_ff = 2048， **正好是 4 倍**。我们等下会专门讲为什么要这样设计。

![FFN第一层线性变换代码输出图](https://file1.kamacoder.com/i/algo/article12_421_p1.png)

## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E7%AC%AC%E4%B8%80%E6%AD%A5-%E5%87%86%E5%A4%87%E8%BE%93%E5%85%A5) 第一步：准备输入

FFN 是接在 Attention 之后的，所以它的输入就是 Attention 子层的输出。

为了方便演示，我们还是用"远方有颗苹果树"这句话，序列长度 L = 7，d\_model = 8（实际中是 512，这里缩小便于观察）。
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E7%AC%AC%E4%BA%8C%E6%AD%A5-%E7%AC%AC%E4%B8%80%E5%B1%82%E7%BA%BF%E6%80%A7%E5%8F%98%E6%8D%A2-%E5%8D%87%E7%BB%B4) 第二步：第一层线性变换（升维）
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E7%AC%AC%E4%B8%89%E6%AD%A5-%E6%BF%80%E6%B4%BB%E5%87%BD%E6%95%B0-%E5%BC%95%E5%85%A5%E9%9D%9E%E7%BA%BF%E6%80%A7) 第三步：激活函数（引入非线性）

这是 FFN 最关键的一步。为什么必须加激活函数？

如果没有它，两层线性变换叠在一起，本质上还是一个线性变换：

(xW1)W2=x(W1W2)=xW′(x W\_1)W\_2 = x(W\_1 W\_2) = x W'
(xW1​)W2​=x(W1​W2​)=xW′
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E7%AC%AC%E5%9B%9B%E6%AD%A5-%E7%AC%AC%E4%BA%8C%E5%B1%82%E7%BA%BF%E6%80%A7%E5%8F%98%E6%8D%A2-%E9%99%8D%E7%BB%B4) 第四步：第二层线性变换（降维）
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E4%B8%BA%E4%BB%80%E4%B9%88%E8%A6%81%E6%89%A9-4-%E5%80%8D) 为什么要扩 4 倍？
...
- Attention 部分：4 个矩阵（WQ,WK,WV,WOW\_Q, W\_K, W\_V, W\_OWQ​,WK​,WV​,WO​），每个 dmodel×dmodeld\_{model} × d\_{model}dmodel​×dmodel​，共 4×dmodel24 × d\_{model}^24×dmodel2​
- FFN 部分：2 个矩阵，dmodel×dff+dff×dmodel=2×dmodel×dffd\_{model} × d\_{ff} + d\_{ff} × d\_{model} = 2 × d\_{model} × d\_{ff}dmodel​×dff​+dff​×dmodel​=2×dmodel​×dff​

当 dff=4×dmodeld\_{ff} = 4 × d\_{model}dff​=4×dmodel​ 时，FFN 参数量 = 8×dmodel28 × d\_{model}^28×dmodel2​， **恰好是 Attention 的 2 倍**。
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E5%AE%8C%E6%95%B4%E4%BB%A3%E7%A0%81-%E6%8A%8A%E5%9B%9B%E6%AD%A5%E6%89%93%E5%8C%85%E6%88%90%E4%B8%80%E4%B8%AA%E7%B1%BB) 完整代码：把四步打包成一个类
...
## [\#](https://notes.kamacoder.com/llm/transformer/fnn_code.html\#%E4%B8%89%E6%AD%A5%E6%B5%81%E7%A8%8B) 三步流程

| 步骤 | 操作 | 输入形状 | 输出形状 |
| --- | --- | --- | --- |
| ① 升维 | x⋅W1+b1x · W\_1 + b\_1x⋅W1​+b1​ | (L,dmodel)(L, d\_{model})(L,dmodel​) | (L,dff)(L, d\_{ff})(L,dff​) |
| ② 激活 | ReLU / GELU | (L,dff)(L, d\_{ff})(L,dff​) | (L,dff)(L, d\_{ff})(L,dff​) |
| ③ 降维 | h⋅W2+b2h · W\_2 + b\_2h⋅W2​+b2​ | (L,dff)(L, d\_{ff})(L,dff​) | (L,dmodel)(L, d\_{model})(L,dmodel​) |
