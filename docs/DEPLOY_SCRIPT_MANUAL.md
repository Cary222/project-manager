...
- 先确保远端 `origin/main` 是你期望部署的分支
- 如果系统里已经配置了 `crontab` 定时任务，例如每 5 分钟执行一次 `bash scripts/deploy.sh`，请确认它正在按预期运行；如果你打算手动执行部署，最好先确认没有另一个同样的定时任务同时触发，避免并发部署
...
- 由维护人员手工触发部署
- `main` 分支即生产分支
- 使用 `crontab` 做定期检查，但只在检测到代码变化时才重启
...
如果服务器上已经有一个固定的系统层面定时任务，比如每 5 分钟执行一次的检查任务，那么建议把这类任务保留在系统定时器里，部署脚本只负责“更新代码 + 构建 + 重启服务”，并且只在检测到 `HEAD` 与 `origin/main` 不一致时才真正执行重启，避免把定时检查和重复重启混在一起。

这种做法的优点是职责清晰、部署流程更短，脚本只关心发布，不会因为额外的定时任务变复杂；同时也能降低部署时误触发业务任务的风险。缺点是定时任务和部署流程仍然是两个独立入口，如果没有统一的执行顺序管理，就需要维护人员自己确保定时任务与发布动作不冲突，必要时还要在部署前确认任务没有正在运行。
...
现有的 `[docs/OPERATIONS.md](docs/OPERATIONS.md)` 已经说明了环境变量、首次部署、systemd 和手动重启方式。本手册补充的是 `scripts/deploy.sh` 的具体运行机制、风险和升级方向。两者建议并存：
