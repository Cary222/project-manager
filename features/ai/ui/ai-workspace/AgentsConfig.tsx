...
function profileKey(
  profile: Pick<SubagentProfile, "scope" | "name">,
): string {
...
function isWritableScope(
  scope: SubagentScope,
): scope is SubagentWritableScope {
...
export function AgentsConfig({
  ...
  const loadProfiles = useCallback(
    async (preferredKey?: string) => {
      try {
        ...
        const data = (await response.json()) as Partial<SubagentProfilesResponse> & {
          error?: string;
        };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const data = (await response.json()) as Partial<SubagentSettingsResponse> & {
          error?: string;
        };
  ...
  const remove = async () => {
    ...
    if (
      !window.confirm(
        t("agents.deleteConfirm", { name: selected.displayName }),
      )
    )
      return;
    setSaving(true);
  ...
  const selectedModelAvailable =
    !draft.model ||
    modelOptions.some((model) => `${model.provider}/${model.id}` === draft.model);
  const selectedModel = (() => {
              ...
              <ConfigDetailStack>
                ...
                >
                  ...
                  <Toggle
                    ...
                    onChange={(checked) =>
                      update("runInBackground", checked)
                    }
                  />
                </div>
              </ConfigDetailStack>
