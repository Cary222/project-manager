...
/** URL 改写时需要保留的 UI 级查询参数（如 fullscreen 全屏模式）。 */
const PRESERVED_QUERY_PARAMS = ["fullscreen"];

/**
 * 构造设置/清除 session 参数的 URL，同时保留 UI 级参数（如 ?fullscreen=1），
 * 避免会话切换时把显示模式参数冲掉。
 */
function buildSessionUrl(sessionId: string | null): string {
...
export function AppShell() {
  return (
    <I18nProvider>
      {/* pi-web-root：pi-web.css 的全局样式已 scope 到此类下，
          避免污染主站（侧边栏 / 顶部导航）的样式 */}
      <div className="pi-web-root">
        <AppShellContent />
      </div>
    </I18nProvider>
  );
...
function AppShellContent() {
    ...
    void fetch("/api/sessions")
      ...
      .then((d) => {
        ...
        if (new URLSearchParams(window.location.search).get("session") !== s.id) {
          router.replace(buildSessionUrl(s.id), { scroll: false });
        }
      })
      .catch(() => {
  ...
  const handleCwdChange = useCallback((
    ...
    router.replace(buildSessionUrl(null), { scroll: false });
  ...
  const handleSelectSession = useCallback((session: SessionInfo, isRestore = false) => {
    ...
    if (!isRestore) {
      router.replace(buildSessionUrl(session.id), { scroll: false });
    }
  ...
  const handleSessionForked = useCallback((newSessionId: string) => {
    ...
    router.replace(buildSessionUrl(newSessionId), { scroll: false });
  }, [invalidateWorkspaceRestore, router, hydrateSelectedSession]);
  ...
  // Priority: explicit new-session cwd > selected session's cwd > active cwd (only when no session selected).
  // Using selectedSession.cwd on refresh ensures the server-side cache key matches the cached
  // entry from the previous request (which was keyed by session.cwd), so page refresh can hit
  // the models-cache without a redundant cold load.
  const effectiveNewSessionCwd = newSessionCwd ?? selectedSession?.cwd ?? (selectedSession === null && activeCwd ? activeCwd : null);
  ...
  const windowTitle = activeCwdName ? `${activeCwdName} - ai-workspace` : "ai-workspace";
        ...
        {([
          {
            ...
            onClick: () => { setModelsConfigOpen(true); setSidebarOpen(false); },
            disabled: false,
            icon: (
          ...
          },
          {
            ...
            onClick: () => { setSkillsConfigOpen(true); setSidebarOpen(false); },
          ...
          },
          {
            ...
            onClick: () => { setPluginsConfigOpen(true); setSidebarOpen(false); },
            disabled: !activeCwd && !selectedSession?.cwd && !newSessionCwd,
            icon: (
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M9 7V2" />
  ...
  const renderChatToolbarActions = (mobile: boolean) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "stretch", height: "100%" }}>
        ...
        {(() => {
          // 默认嵌入模式：工作区渲染在网站侧边栏/顶栏内；
          // ?fullscreen=1 时独占视口。该按钮在两种模式之间切换。
          const embedded = searchParams.get("fullscreen") !== "1";
          const buttonLabel = translate(
            embedded ? "layout.enterFullscreen.label" : "layout.exitFullscreen.label",
          );
          return (
            <button
              type="button"
              onClick={() => {
                // 切换时保留 session 等其他查询参数
                const params = new URLSearchParams(window.location.search);
                if (embedded) params.set("fullscreen", "1");
                else params.delete("fullscreen");
                const qs = params.toString();
                router.push(qs ? `${window.location.pathname}?${qs}` : window.location.pathname);
                if (mobile) setMobileToolbarMoreOpen(true);
              }}
              title={translate(embedded ? "layout.enterFullscreen" : "layout.exitFullscreen")}
              ...
              data-mobile-toolbar-action={mobile ? "fullscreen-toggle" : undefined}
            >
              {embedded ? (
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        {mobile ? (
          <>
            {tokens && tokens.input > 0 && (
              <span className="mobile-session-stat-io" style={{ display: "flex", alignItems: "center", gap: 2, flexShrink: 1 }}>
            ...
            {costText && (
              <span className="mobile-session-stat-cost" style={{ color: "var(--text)", fontWeight: 500, flexShrink: 1 }}>
                {costText}
              </span>
            )}
            {mobileContextText && (
              <span style={{ color: contextColor, flexShrink: 1 }}>
                {mobileContextText}
              </span>
            )}
            {!hasMobileValues && showChat && (
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", color: "var(--text-dim)", flexShrink: 1 }}>
          ...
          <>
            {tokens && tokens.input > 0 && (
              <span style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 1 }}>
            ...
            {costText && (
              <span style={{ display: "flex", alignItems: "center", color: "var(--text)", fontWeight: 500, flexShrink: 1 }}>
                {costText}
              </span>
            )}
            {desktopContextText && (
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: contextColor, flexShrink: 1 }}>
                <svg width="12" height="12" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  return (
    ...
    {modelsConfigOpen && <ModelsConfig onClose={() => { setModelsConfigOpen(false); setModelsRefreshKey((k) => k + 1); }} onModelsChanged={() => setModelsRefreshKey((k) => k + 1)} />}
