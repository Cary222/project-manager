...
function previewForEntry(entry: ProjectableEntry): MessagePreview | undefined {
...
export function projectTreeForResponse<T extends ProjectableTreeNode<T>>(
  ...
  const cloneNode = (node: T, compressedEntryIds?: string[], messagePreview?: MessagePreview): T => ({
    ...node,
    children: [],
  ...
  const appendFlattenedKeptDescendants = (source: T, projectedParent: T) => {
    const pending = [{
      ...
      messagePreview: undefined as MessagePreview | undefined,
    }];
    const flattenedSeen = new Set<T>();

    while (pending.length > 0) {
      const { node, compressedEntryIds, messagePreview } = pending.pop()!;
      if (flattenedSeen.has(node)) continue;
      flattenedSeen.add(node);
      const nextPreview = messagePreview ?? previewForEntry(node.entry);
      ...
      for (let i = node.children.length - 1; i >= 0; i--) {
        pending.push({
          ...
          messagePreview: keep.has(node) ? undefined : nextPreview,
        });
      }
    }
  };
  ...
  while (tasks.length > 0) {
    ...
    for (const sourceChild of source.children) {
      ...
      let messagePreview = previewForEntry(child.entry);
      while (!keep.has(child) && child.children.length === 1) {
        compressedEntryIds.push(child.entry.id);
        child = child.children[0];
        messagePreview ??= previewForEntry(child.entry);
      }
      ...
      const projectedChild = cloneNode(child, compressedEntryIds, messagePreview);
