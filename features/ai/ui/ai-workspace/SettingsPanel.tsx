...
import { EmbeddedRulesConfig } from "./RulesConfig";
import { ConfigSwitch } from "./SettingsUi";

export type ExtendedSettingsSection = SettingsSection | "rules";

interface Props {
  cwd: string | null;
  sessionId: string | null;
  initialSection: ExtendedSettingsSection;
  onClose: () => void;
...
export function SettingsPanel({
  ...
  const [section, setSection] =
    useState<ExtendedSettingsSection>(initialSection);
  const [mountedSections, setMountedSections] = useState<
    ReadonlySet<ExtendedSettingsSection>
  >(() => new Set([section]));
  const sections: {
    id: ExtendedSettingsSection;
    label: string;
    ...
    { id: "rules", label: t("common.rules") || "Rules", requiresProject: true },
  ...
  const effectiveSection =
    !cwd && (section === "skills" || section === "plugins" || section === "rules")
      ? "general"
      : section;

  const activateSection = (nextSection: ExtendedSettingsSection) => {
    setMountedSections((current) => new Set(current).add(nextSection));
    setSection(nextSection);
    if (nextSection !== "rules") {
      setLastSettingsSection(nextSection as SettingsSection);
    }
  };

  const sectionHost = (id: ExtendedSettingsSection, content: ReactNode) =>
    mountedSections.has(id) ? (
      <div
        key={id}
        hidden={effectiveSection !== id}
      ...
      <div className="settings-dialog-surface">
        <div className="settings-dialog-header">
          ...
          <select
            ...
            onChange={(event) =>
              activateSection(event.target.value as ExtendedSettingsSection)
            }
          ...
          >
            {sections.map((item) => {
              const selected = effectiveSection === item.id;
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  title={
                    disabled ? t("settings.projectRequired") : item.label
                  }
