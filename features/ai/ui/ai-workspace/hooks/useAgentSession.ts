...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const pendingNavigateLeafRef = useRef<string | null>(null);
  ...
  const sessionStats = useMemo(() => {
    ...
    for (const msg of messages) {
      ...
      toolCalls += (msg as AssistantMessage).content.filter(
        (c) => c.type === "toolCall",
      ).length;
      if (!u) continue;
  ...
  const loadSystemPrompt = useCallback(async () => {
    ...
    const state = await sendAgentCommand<AgentStateResponse>(sid, {
      type: "get_state",
    });
    ...
    async (message: string, images?: AttachedImage[]) => {
      ...
      try {
        if (isNew && newSessionCwd) {
          ...
          const pendingLeaf = pendingNavigateLeafRef.current;
          if (pendingLeaf) {
            pendingNavigateLeafRef.current = null;
            await sendAgentCommand(session.id, {
              type: "navigate_tree",
              targetId: pendingLeaf,
            });
            // Truncate the view to the navigation point so the new prompt+response
            // render in the correct position (matching the server context).
            await loadContext(session.id, pendingLeaf);
          }
          promptRequestStarted = true;
          await sendAgentCommand(session.id, {
            type: "prompt",
        ...
        const canceled =
          e instanceof AgentEventConnectionError && e.status === "canceled";
        // An intentional close (new conversation / session switch / unmount)
        // while the stream was still connecting cancels the send — the prompt
        // was never dispatched, so restore the draft quietly instead of
        // alarming the user with a connection-failure notice.
        if (!canceled) console.error("Failed to send message:", e);
  ...
  const handleNavigate = useCallback(async (entryId: string) => {
    ...
    const sid = sessionIdRef.current;
    if (!sid) return;
    // "Edit from here" is a draft, not a commit: hold the tree navigation
    // until the edited message is actually sent. Dismissing the edit (without
    // sending) must leave the conversation untouched. handleSend applies
    // pendingNavigateLeafRef before dispatching the prompt.
    pendingNavigateLeafRef.current = entryId;
