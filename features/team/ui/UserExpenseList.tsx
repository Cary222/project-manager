...
type Props = {
  expenses: MonthlyExpenseWithUser[];
  userName: string;
  isOwnProfile: boolean;
};

export function UserExpenseList({ expenses, userName, isOwnProfile }: Props) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-ink-300 bg-white p-12 text-center">
        ...
        <p className="mt-1 text-sm text-ink-500">
          {isOwnProfile ? "开始记录你的第一笔报销吧" : `${userName} 还没有提交报销`}
        </p>
        {isOwnProfile && (
          <Link
  ...
  return (
    <div className="space-y-3 pm-fade-in">
      {expenses.map((expense) => {
        ...
        const participantCount = 1 + otherShares.length;
