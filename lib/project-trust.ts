...
export async function getProjectTrustStatus(cwd: string, agentDir: string): Promise<ProjectTrustStatus> {
  const { hasTrustRequiringProjectResources, ProjectTrustStore } = await import("@earendil-works/pi-coding-agent");
...
export async function trustProject(cwd: string, agentDir: string): Promise<ProjectTrustStatus> {
  const { ProjectTrustStore } = await import("@earendil-works/pi-coding-agent");
  const status = await getProjectTrustStatus(cwd, agentDir);
 ...
 */
export async function projectTrustReloadOptions(
  cwd: string,
  agentDir: string,
): Promise<{ resolveProjectTrust: () => Promise<boolean> } | undefined> {
  const { ProjectTrustStore } = await import("@earendil-works/pi-coding-agent");
  const status = await getProjectTrustStatus(cwd, agentDir);
