/**
 * Model Catalog — 公共模型元数据与价格
 *
 * =============================================================================
 * 职责边界
 * =============================================================================
 * ✅ 负责：
 *   - models.dev 解析（flattenModelsDevCatalog）
 *   - 模型价格推荐（recommendModelCatalogPreset）
 *   - 模型搜索（searchModelCatalog）
 *   - 价格共识计算（consensus pricing）
 *   - Provider/Model 匹配（providerMatches, baseUrlMatches）
 *
 * ❌ 不负责：
 *   - 模型发现（由 registry.ts / model-discovery.ts 提供）
 *   - 模型创建（由 registry.ts 提供）
 *   - 凭证管理（由 api-key-store.ts 提供）
 *   - 模型 scope 解析（由 model-scope.ts 提供）
 *
 * =============================================================================
 * Catalog vs Discovery
 * =============================================================================
 * Catalog 和 Discovery 是两个正交的职责：
 *
 * Discovery：Provider API → 真实可用模型
 *   - 回答："这个 Provider 有哪些模型可用？"
 *   - 返回：模型 ID、名称、provider
 *
 * Catalog：models.dev → 元数据 / 价格 / 能力
 *   - 回答："这个模型的价格、context window、能力是什么？"
 *   - 返回：cost、contextWindow、reasoning、input modalities
 *
 * Catalog 不依赖 Discovery：
 *   - Catalog 可以独立工作（基于静态 models.dev 数据）
 *   - Discovery 可以独立工作（基于 Provider API 响应）
 *   - 两者可以组合：Discovery 获取模型列表 + Catalog 补充元数据
 *
 * =============================================================================
 * 数据来源
 * =============================================================================
 * Catalog 的数据来自 models.dev 文件（公共模型目录）：
 * - 包含 100+ 主流模型的元数据
 * - 包含价格信息（input/output cost）
 * - 包含能力信息（reasoning、context window、modalities）
 * - 由社区维护，定期更新
 *
 * =============================================================================
 * 使用场景
 * =============================================================================
 * 1. Model Selector UI：显示模型价格、能力
 * 2. Cost Estimation：估算对话成本
 * 3. Model Recommendation：根据能力推荐模型
 * 4. Model Search：根据名称/provider 搜索模型
 *
 * =============================================================================
 * 重要：Catalog 数据是只读的
 * =============================================================================
 * Catalog 提供的是参考数据，不是权威数据：
 * - 价格可能与实际 Provider 定价不一致
 * - 能力可能与实际模型能力不一致
 * - 应作为 UI 展示和推荐使用，不应作为计费依据
 */
...
// ── models.dev 目录加载（共享）──────────────────────────────────
// 原实现位于 app/api/models-config/catalog/route.ts，Stage 6 下沉至此供
// User Scope 元数据富化复用；全局缓存 key 与 TTL 保持原语义。
...
declare global {
  var __piModelsDevCatalogCache: ModelsDevCatalogCache | undefined;
}

function getModelsDevCatalogCache(): ModelsDevCatalogCache {
...
/** 加载 models.dev 目录（1 小时缓存 + in-flight 去重；失败时回落旧缓存）。 */
export async function loadModelsDevCatalog(): Promise<ModelCatalogEntry[]> {
  ...
  if (!cache.inFlight) {
    cache.inFlight = fetchModelsDevCatalog().then((entries) => {
      ...
      cache.expiresAt = Date.now() + MODELS_DEV_TTL_MS;
      return entries;
    }).finally(() => {
