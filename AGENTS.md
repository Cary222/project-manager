...
| ⚖️ **工程原则（必读）** | [.cursor/rules/Pragmatic-Engineering-Rule.mdc](.cursor/rules/Pragmatic-Engineering-Rule.mdc)（alwaysApply）：中文回复；架构讨论先质疑假设、防过度设计；方向批准后停止争论、专注交付可上线的实现 |
